---
title: "(07WFE) Assignment WFEZhouNa"
...
```{r}
sum_of_vec <- function(vec, sum_of = sum){
  clean_vec <- vec[!is.na(vec)]
  result <- sum_of(clean_vec)
  return(result)
}
...
```{r}
mtrx <- matrix(c(1:25), nrow=5, ncol=5)
sum_matrix <- function(m){
  result <- 0
  probar <- txtProgressBar(min=0, max=length(m), style=3, char="*")
  for (i in 1:nrow(m)){
    for (j in 1:ncol(m)){
      result <- result+m[i,j]
      Sys.sleep(0.25)
      setTxtProgressBar(probar, (i - 1) * ncol(m) + j)
    }
  }
  close(probar)
  return(result)
...
result <- try(withdrawal(-300,200), silent = TRUE)
print(result) 

result <- try(withdrawal(300, 200), silent = TRUE)
print(result) 

result <- try(withdrawal(100, 200), silent = TRUE)
print(result) 
```
